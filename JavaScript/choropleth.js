...
fetch('../datasets/us-states.json')
  ...
  .then(us => {
    fetch('../datasets/density_per_sqkm.csv')
...
function drawMap(us, data) {
  ...
  svg.selectAll('path')
    ...
    .on('mouseover', function(d) {
      ...
      var count = stateData ? stateData.density_per_sq_km : 'Data unavailable';
      tooltip.transition()
        .duration(200)
        .style('opacity', .9);
  ...
  var tooltip = d3.select('body')
    ...
    function getColor(data) {
        var count = parseFloat(data.density_per_sq_km);
        ...
        var ranges = [0.0001, 0.001, 0.01,0.1, 1, 2, 3, 4, 5, 6];
  ...
  var legend = svg.append('g')
    .attr('class', 'legend')
    .attr('transform', 'translate(' + (width - 60) + ', 20)'); // Adjust the legend position as needed

  var legendColors = ['#FFFFFF', '#E6E6E6', '#CCCCCC', '#B3B3B3', '#999999', '#808080', '#666666', '#4D4D4D', '#333333', '#1A1A1A'];
  var legendText = ['0.0001', '0.001','0.01', '0.1', '1', '2', '3', '4', '5', '6'];
